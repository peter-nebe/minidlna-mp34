...
CreateDatabase(void)
{
	...
	const char *containers[] = { "0","-1",   "root",
	                 ...
	                 IMAGE_CAMERA_ID, IMAGE_ID, _("Camera"),
                      ...
                      MP3ASMP4_ID, "0", _("Play mp3 as mp4"),
			0 };
...
/*
 * insertMp3AsMp4* functions:
 * Copyright (c) 2020 Peter Nebe (mail@peter-nebe.dev)
 */
static void insertMp3AsMp4File(const char *mp3Path /*without extension*/, const char *title, const char *duration,
                               const char *parentId, int objectId)
{
  char *mp4Path = 0;
  if(xasprintf(&mp4Path, "%s" STR_MP4CACHE "%s.mp4", db_path, mp3Path) > 0)
  {
    const char *titleSuffix = STR_PLAY;
    struct stat file = { 0 };
    if(stat(mp4Path, &file))
    {
      stat(conversion_info_path, &file);
      titleSuffix = STR_CONVERT;
    }
    char *const mp3PathCopy = strdup(mp3Path);
    if(mp3PathCopy)
    {
      const char *const name = basename(mp3PathCopy);
      char *detailTitle = 0;
      if(xasprintf(&detailTitle, "%s%s", title ? title : name, titleSuffix) > 0)
      {
        if(sql_exec(db, "INSERT INTO DETAILS (PATH, SIZE, TIMESTAMP, TITLE, DURATION, RESOLUTION, DLNA_PN, MIME) "
                        "VALUES (%Q, %lld, %lld, %Q, %Q, '1920x1080', 'AVC_MP4_HP_HD_AAC', 'video/mp4')",
                        mp4Path, (int64_t)file.st_size, (int64_t)file.st_mtime, detailTitle, duration) == SQLITE_OK)
        {
          const int64_t detailId = sqlite3_last_insert_rowid(db);
          char *objName = 0;
          if(xasprintf(&objName, "%s%s", name, titleSuffix) > 0)
          {
            sql_exec(db, "INSERT INTO OBJECTS (OBJECT_ID, PARENT_ID, CLASS, DETAIL_ID, NAME) "
                         "VALUES ('%s$%X', %Q, 'item.videoItem', %lld, %Q)",
                         parentId, objectId, parentId, detailId, objName);
            free(objName);
          }
        }
        else
...
static int insertMp3AsMp4(const char *srcFolderId, const char *destFolderId);

static void insertMp3AsMp4Folder(const char *srcFolderId, const char *refId, const char *srcDetailId,
                                 const char *name, const char *parentId, int *newObjId)
{
  char *folderId = 0;
  if(xasprintf(&folderId, "%s$%X", parentId, (*newObjId)++) > 0)
  {
    if(sql_exec(db, "INSERT INTO OBJECTS (OBJECT_ID, PARENT_ID, REF_ID, CLASS, DETAIL_ID, NAME) "
                    "VALUES (%Q, %Q, %Q, 'container.storageFolder', %Q, %Q)",
                    folderId, parentId, refId, srcDetailId, name) == SQLITE_OK)
    {
      if(insertMp3AsMp4(srcFolderId, folderId) > 1)
      {
        // folder contains more than one mp3 item (directly, not in any subfolder) -> convert folder as a whole
        int rows = 0;
        char **result = 0;
        char *sql = sqlite3_mprintf("SELECT PATH, TITLE FROM DETAILS WHERE ID = %s", srcDetailId);
        if(sql_get_table(db, sql, &result, &rows, 0) == SQLITE_OK)
        {
          if(rows == 1)
            insertMp3AsMp4File(result[2], result[3], 0, parentId, (*newObjId)++);
          sqlite3_free_table(result);
        }
        sqlite3_free(sql);
      }
...
static int insertMp3AsMp4(const char *srcFolderId, const char *destFolderId)
{
  ...
  int rows = 0;
  char **result = 0;
  char *sql = sqlite3_mprintf("SELECT PATH, TITLE, DURATION FROM OBJECTS JOIN DETAILS ON OBJECTS.DETAIL_ID = DETAILS.ID "
                              "WHERE PARENT_ID = %Q AND CLASS = 'item.audioItem.musicTrack' AND lower(substr(PATH, -4)) = '.mp3'",
                              srcFolderId);
  if(sql_get_table(db, sql, &result, &rows, 0) == SQLITE_OK)
  {
    for(int i = 0, ind = 3; i < rows; i++, ind += 3)
    {
      char *const path = result[ind];
      path[strlen(path) - 4] = '\0'; // strip '.mp3'
      insertMp3AsMp4File(path, result[ind + 1], result[ind + 2], destFolderId, newObjId++);
    }
    sqlite3_free_table(result);
  }
  sqlite3_free(sql);

  const int numberOfMp3Items = rows;
  rows = 0;
  result = 0;
  sql = sqlite3_mprintf("SELECT OBJECT_ID, REF_ID, DETAIL_ID, NAME FROM OBJECTS "
                        "WHERE PARENT_ID = %Q AND CLASS = 'container.storageFolder'",
                        srcFolderId);
  if(sql_get_table(db, sql, &result, &rows, 0) == SQLITE_OK)
  {
    for(int i = 0, ind = 4; i < rows; i++, ind += 4)
    {
      insertMp3AsMp4Folder(result[ind], result[ind + 1], result[ind + 2], result[ind + 3], destFolderId, &newObjId);
    }
    sqlite3_free_table(result);
  }
  sqlite3_free(sql);

  return numberOfMp3Items;
}
...
//	av_register_all(); // omit this function call in ffmpeg 4.0 and later (see https://github.com/leandromoreira/ffmpeg-libav-tutorial/issues/29)
	...
	insertMp3AsMp4(MUSIC_DIR_ID, MP3ASMP4_ID);
